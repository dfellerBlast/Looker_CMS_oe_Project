view: pre_oe_weekly_planfindermetrics {derived_table: {
    sql:WITH plan_compare AS (
      ...
      ,user_agg_2019 AS (SELECT week_of_year, CONCAT(PARSE_DATE('%Y%m%d', MIN(date)), ' - ', PARSE_DATE('%Y%m%d', MAX(date))) AS date_range, year
      ,AVG(is_new) AS new_user_percent
      FROM user_data
      WHERE year = 2019
      GROUP BY week_of_year, year)
      ...
      -- ma and pdp sessions
      ,CASE WHEN COUNTIF(REGEXP_CONTAINS(hits.eventinfo.eventLabel, 'MAP')
      ...
      ,CASE WHEN COUNTIF(hits.eventInfo.eventCategory = 'MCT' AND hits.eventInfo.eventAction = 'Find Plans Landing Page - Login' AND hits.eventInfo.eventLabel = 'Continue without logging in 1') > 0 THEN 1 ELSE 0 END AS anonymous
      --insulin demo click
      ,COUNTIF(hits.eventinfo.eventLabel = 'Insulin Savings Program') AS insulin_demo_click
      --enroll
      ...
      ,SUM(plan_results) / SUM(interact) AS plan_results_nonbounce_perc
      FROM sessions
      WHERE sessionId IN (SELECT sessionId FROM plan_compare) AND year=2019
      GROUP BY week_of_year, year)
      ...
      ,qualtrics AS (
          ...
          AND (start_date BETWEEN '2020-10-15' AND '2020-12-07' OR start_date BETWEEN '2019-10-15' AND '2019-12-07')
      ...
      LEFT JOIN etl_enroll ON etl_enroll.week_of_year = user_agg_2020.week_of_year
      LEFT JOIN qualtrics ON qualtrics.week_of_year = user_agg_2020.week_of_year
      LEFT JOIN medigap_wizard_agg ON medigap_wizard_agg.week_of_year = user_agg_2020.week_of_year
      WHERE user_agg_2020.year = 2020 AND etl_enroll.year = 2020 AND qualtrics.year = 2020 AND medigap_wizard_agg.year = 2020
      ORDER BY Week)

      -- 2019
      , full_table_2019 AS(SELECT CONCAT('Week ', user_agg_2019.week_of_year-40) AS Week, user_agg_2019.date_range
      ,CAST(sessions AS FLOAT64) AS Sessions, CAST(users AS FLOAT64) AS Users, CAST(pageviews AS FLOAT64) AS Pageviews
      ,ROUND(bounce_rate * 100) AS `Bounce Rate`
      ,ROUND(sessions_per_user, 2) AS `Sessions Per User`
      ,ROUND(new_user_percent * 100) AS `% New Users`, ROUND(mobile_users * 100) AS `% Mobile Users`
      ...
      ,ROUND(will_contact_cc * 100) AS `Will Contact Call Center`
      FROM user_agg_2019
      LEFT JOIN session_agg_2019 ON session_agg_2019.week_of_year = user_agg_2019.week_of_year
      LEFT JOIN etl_enroll ON etl_enroll.week_of_year = user_agg_2019.week_of_year AND etl_enroll.year = user_agg_2019.year
      LEFT JOIN qualtrics ON qualtrics.week_of_year = user_agg_2019.week_of_year AND qualtrics.year = user_agg_2019.year
      LEFT JOIN medigap_wizard_agg ON medigap_wizard_agg.week_of_year = user_agg_2019.week_of_year AND medigap_wizard_agg.year = user_agg_2019.year
      -- WHERE user_agg_2019.year = 2019 AND etl_enroll.year = 2019 AND qualtrics.year = 2020 AND medigap_wizard_agg.year = 2019
      ORDER BY Week)
      ...
      ,t_2019 AS (
      SELECT * FROM full_table_2019
      UNPIVOT(values_2019 FOR metric IN (Sessions, Users, Pageviews, `Bounce Rate`, `Sessions Per User`, `% New Users`, `% Mobile Users`, `Logged In %`, `Anonymous %`, `PDP PlanType Clicks %`
      ,`MA PlanType Clicks %`, `MA & PDP PlanType Clicks %`, `Insulin Demo Filter Clicks (Total)`, `Enroll All Sessions %`, `Enroll Non-Bounce %`, `Plan Results All Sessions %`
      ,`Plan Results Non-Bounce %`, `Wizard Sessions`, `Wizard Conversions %`, `Medigap Sessions`, `Medigap Conversion %`,`Online Enrollments`,`Call Center Enrollments`
      ,`Total Enrollments`, `Overall CSAT`,`Goal Completion %`, `Will Contact Call Center`))
      )

      SELECT t_2020.Week, t_2020.date_range, t_2020.metric
      ,CASE WHEN t_2020.metric IN ('Sessions', 'Users', 'Pageviews', 'Insulin Demo Filter Clicks (Total)', 'Wizard Sessions', 'Medigap Sessions', 'Online Enrollmetns', 'Call Center Enrollments', 'Total Enrollments')
              -- THEN CONCAT(FORMAT("%'d", CAST(values_2020 AS int64)), SUBSTR(FORMAT("%.2f", CAST(values_2020 AS float64)), -3))
